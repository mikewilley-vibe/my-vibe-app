    ...
    return (
      <div className="min-h-screen pb-16" style={{ backgroundImage: uvaPageBg() }}>
        <section className="mx-auto max-w-6xl px-4 py-10 sm:py-12">
          <UvaTabs />
          <h1 className="font-display text-3xl font-semibold text-slate-900">UVA Basketball Results</h1>
          <p className="mt-4 text-sm text-slate-500">Basketball results are temporarily unavailable.</p>
        </section>
      </div>
    );
  }
    ...
    <div className="min-h-screen pb-16" style={{ backgroundImage: uvaPageBg() }}>
      <section className="mx-auto max-w-6xl px-4 py-10 sm:py-12">
        ...
        <UvaPageHeader
          ...
          description={`Last updated ${updatedAt.toLocaleString("en-US", {
            month: "numeric",
            day: "numeric",
            year: "numeric",
            hour: "numeric",
        ...
        <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
          <UvaStat label="Record" value={`${w}–${l}`} />
        ...
        {results.length === 0 ? (
          ...
          <TwoColumnSection className="gap-6">
            <div className="space-y-3">
              {left.map((g) => (
                <UvaResultCard key={`${g.dateIso}-${g.opponent}`} g={g} recapLabel="ESPN recap" />
              ))}
            </div>
            <div className="space-y-3">
              {right.map((g) => (
                <UvaResultCard key={`${g.dateIso}-${g.opponent}-r`} g={g} recapLabel="ESPN recap" />
              ))}
            </div>
          </TwoColumnSection>
        )}
