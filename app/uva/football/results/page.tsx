...
function parseResultScore(score: string) {
  // expected "W 27–24" or "L 17–20"
  const s = String(score ?? "").trim();
  const wl = s.startsWith("W") ? "W" : s.startsWith("L") ? "L" : null;

  // grab first two numbers in string
    ...
    return (
      <main className="min-h-screen bg-slate-50">
        <section className="mx-auto max-w-5xl px-4 py-10">
          <h1 className="text-3xl font-bold text-slate-900">UVA Football Results</h1>
      ...
      <section className="mx-auto max-w-5xl px-4 py-12">
        ...
        <div className="mb-6 space-y-4">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            <Stat label="2025 Record" value={`${w}–${l}`} />
        ...
        {results.length === 0 ? (
          ...
          <TwoColumnSection className="gap-6">
            <div className="space-y-3">{left.map((g) => <ResultCard key={`${g.dateIso}-${g.opponent}`} g={g} />)}</div>
            <div className="space-y-3">{right.map((g) => <ResultCard key={`${g.dateIso}-${g.opponent}-r`} g={g} />)}</div>
          </TwoColumnSection>
        )}
      </section>
    </main>
